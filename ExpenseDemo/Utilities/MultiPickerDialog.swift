...
class MultiPickerDialog: UIView, UITableViewDelegate, UITableViewDataSource {
    ...
    typealias MultiPickerCallback = (_ values: [CategoryModel]) -> Void
    ...
    /* Variables */
    private var pickerData =         [CategoryModel]()
    ...
    func setupView() {
        ...
        picker.register(UITableViewCell.self, forCellReuseIdentifier: "cell")
    ...
    func findIndicesForValues(values: [String], array: [CategoryModel]) -> [Int] {
        var selectedIndices : [Int] = []
        for (index, dictionary) in array.enumerated() {
            for selectedOption in values {
                if "\(dictionary.id)" == selectedOption {
    ...
    func show(title: String, doneButtonTitle: String = "Select", cancelButtonTitle: String = "Cancel", options: [CategoryModel], selected: [String]? = nil, callback: @escaping MultiPickerCallback) {
        ...
        Constants.APP_DELEGATE.window?.addSubview(self)
        Constants.APP_DELEGATE.window?.bringSubviewToFront(self)
        Constants.APP_DELEGATE.window?.endEditing(true)
    ...
    private func createContainerView() -> UIView {
        ...
        //Title
        self.titleLabel = UILabel(frame: CGRect.init(x: 10, y: 10, width: 280, height: 50))
...
//        self.titleLabel.backgroundColor = .white
        ...
        self.picker = UITableView(frame: CGRect.init(x: 0, y: 50, width: 10, height: 10))
        ...
        self.picker.frame.size.height = 450
    ...
    @objc func buttonTapped(sender: UIButton!) {
        if sender.tag == kPickerDialogDoneButtonTag {
            var theSelectedValues : [CategoryModel] = []
            ...
            if let indexPathsForSelectedRows = self.picker.indexPathsForSelectedRows{
                for indexPath  in indexPathsForSelectedRows{
                    ...
                    let categoryObj = CategoryModel.init(id: Int16(cell?.contnetIdentifier ?? "0") ?? 0, name: cell?.textLabel?.text ?? "")
                    theSelectedValues.append(categoryObj)
                }
            }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        theCell.textLabel?.text = self.pickerData[indexPath.row].name
        theCell.contnetIdentifier = "\(self.pickerData[indexPath.row].id)"
        theCell.textLabel?.textAlignment = .left
        theCell.backgroundColor = UIColor.clear
        theCell.textLabel?.font = UIFont(name: "AvenirNext", size: 18)
